...
$esSuperAdmin = $_SESSION['es_superadmin'] ?? false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'obtener_estadisticas_dashboard') {
    // Validar token CSRF
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        echo json_encode(['success' => false, 'error' => 'Token de seguridad inválido.']);
        exit;
    ...
    try {
        ...
        // Propiedades activas y cambio
        $propiedadesActivas = $propiedadesVisibles ?? 0;
        ...
        // Solicitudes pendientes y cambio
        ...
        $tasaMesPasado = 84; // Valor de ejemplo - en producción calcularías el real
        
        $estadisticas['tasa_aprobacion'] = $tasaAprobacion;
        $estadisticas['cambio_aprobacion'] = $tasaAprobacion - $tasaMesPasado;
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $labels[] = substr($diaSemana, 0, 3); // Lun, Mar, Mié...
            ...
            // Propiedades publicadas por día
        ...
        $graficos['actividad_diaria'] = [
            ...
            'datasets' => [
                [
                    ...
                    'data' => array_column($actividadDiaria, 'nuevos_usuarios')
                ],
                [
                    'label' => 'Propiedades Publicadas',
                    'data' => array_column($actividadDiaria, 'propiedades_publicadas')
                ],
                [
                    'label' => 'Solicitudes',
                    'data' => array_column($actividadDiaria, 'solicitudes')
                ]
            ]
        ];
        ...
        $metricas = [
            'total_registros' => number_format($totalUsuarios + $totalAprobadas + $totalRechazadas + $solicitudesPendientes, 0, ',', '.'),
            'tiempo_respuesta' => '0.8' // En producción calcularías esto
        ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'obtener_actividad_reciente') {
    // Validar token CSRF
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        echo json_encode(['success' => false, 'error' => 'Token de seguridad inválido.']);
        exit;
...
<div class="layout">
    ...
    <aside class="sidebar">
        ...
        <div class="submenu" id="submenuPropiedades" style="<?= in_array($seccionActiva, ['agregarpropiedad', 'propiedadespublicadas']) ? 'max-height: 240px;' : '' ?>">
            <button class="submenu-btn <?= $seccionActiva === 'agregarpropiedad' ? 'activo' : '' ?>" data-seccion="agregarpropiedad">Agregar Propiedad</button>
            <button class="submenu-btn <?= $seccionActiva === 'propiedadespublicadas' ? 'activo' : '' ?>" data-seccion="propiedadespublicadas">Propiedades Publicadas</button>
        </div>
    ...
    <main class="contenido">
        ...
        <section id="inicio" class="seccion <?= $seccionActiva === 'inicio' ? 'visible' : '' ?>">
            ...
            <div class="dashboard-widgets-grid">
                ...
                <div class="widget-main widget-system">
                    ...
                    <div class="widget-content">
                        <div class="system-metrics">
                            <div class="metric-item">
                                ...
                                <div class="metric-details">
                                    ...
                                    <span class="metric-value" id="totalRegistrosSistema">
                                        <?= number_format($totalUsuarios + ($propiedadesVisibles ?? 0) + ($solicitudesPendientes ?? 0), 0, ',', '.') ?>
                                    </span>
                                </div>
                            </div>
                            <div class="metric-item">
                            ...
                            <div class="metric-item">
                                ...
                                <div class="metric-details">
                                    ...
                                    <span class="metric-value" id="systemSecurity">Alta</span>
                                </div>
                            </div>
                        </div>
                        <div class="system-status">
                            ...
                            <span class="status-time" id="uptimeCounter"><?= date('H:i') ?></span>
                        </div>
                    </div>
                </div>
                            ...
                            try {
                                ...
                                if (!empty($actividadReciente)):
                                    foreach ($actividadReciente as $actividad):
                                        ...
                                        if (strpos($actividad['accion'], 'agregó') !== false) {
                                            $icono = 'fa-plus-circle';
                                            $color = '#28a745';
                                        } elseif (strpos($actividad['accion'], 'editó') !== false) {
                                            $icono = 'fa-edit';
                                            $color = '#17a2b8';
                                        } elseif (strpos($actividad['accion'], 'eliminó') !== false) {
                                            $icono = 'fa-trash';
                                            $color = '#dc3545';
                                        } elseif (strpos($actividad['accion'], 'aprobó') !== false) {
                ...
                <div class="widget-main widget-stats">
                    <div class="widget-header">
                        <h3><i class="fa-solid fa-chart-line"></i> Estadísticas Rápidas</h3>
                        <div class="time-selector">
                            <select id="statsTimeRange" class="time-select">
                        ...
                        <div class="stats-grid">
                            <div class="stat-card">
                                <div class="stat-header">
                                    <i class="fa-solid fa-users" style="color: #4a6cf7;"></i>
                                    <span class="stat-title">Nuevos Usuarios</span>
                                </div>
                            ...
                            <div class="stat-card">
                                <div class="stat-header">
                                    ...
                                    <span class="stat-title">Propiedades</span>
                                </div>
                                <div class="stat-value" id="newPropertiesCount">0</div>
                                <div class="stat-trend trend-up">
                                    <i class="fa-solid fa-arrow-up"></i>
                                    <span id="newPropertiesTrend">0%</span>
                                </div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-header">
                                    <i class="fa-solid fa-handshake" style="color: #f59e0b;"></i>
                                    <span class="stat-title">Solicitudes</span>
                                </div>
                                <div class="stat-value" id="newRequestsCount"><?= $solicitudesPendientes ?? 0 ?></div>
                                <div class="stat-trend trend-down">
                                    <i class="fa-solid fa-arrow-down"></i>
                                    <span id="newRequestsTrend">0%</span>
                                </div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-header">
                                    <i class="fa-solid fa-chart-bar" style="color: #8b5cf6;"></i>
                                    <span class="stat-title">Actividad</span>
                                </div>
                                <div class="stat-value" id="activityCount"><?= $logsHoy ?? 0 ?></div>
                                <div class="stat-trend trend-up">
                                    <i class="fa-solid fa-arrow-up"></i>
                                    <span id="activityTrend">0%</span>
                                </div>
                            </div>
                        </div>
                        <div class="stats-chart">
                            <canvas id="miniStatsChart" height="80"></canvas>
                        </div>
                    </div>
                </div>

                <!-- WIDGET 4: NOTIFICACIONES URGENTES -->
                <div class="widget-main widget-notifications">
                    <div class="widget-header">
                    ...
                    <div class="widget-content">
                        <div class="notifications-list" id="notificationsList">
                            <!-- Notificaciones cargadas por JavaScript -->
                            <div class="notification-item urgent">
                                <div class="notification-icon">
                                    <i class="fa-solid fa-exclamation-triangle"></i>
                                </div>
                                <div class="notification-content">
                                    <span class="notification-title">Solicitudes pendientes</span>
                                    <span class="notification-desc">Tienes <?= $solicitudesPendientes ?? 0 ?> solicitudes por revisar</span>
                                </div>
                                <span class="notification-time">Ahora</span>
                            </div>
                            
                            <?php if ($solicitudesPendientes > 0 && $solicitudesPendientes > 5): ?>
                            <div class="notification-item warning">
                                <div class="notification-icon">
                                    <i class="fa-solid fa-clock"></i>
                                </div>
                                <div class="notification-content">
                                    <span class="notification-title">Solicitudes urgentes</span>
                                    <span class="notification-desc">Hay solicitudes con más de 7 días de espera</span>
                                </div>
                            ...
                            <div class="notification-item info">
                                ...
                                <div class="notification-content">
                                    <span class="notification-title">Usuarios activos</span>
                                    <span class="notification-desc"><?= $usuariosActivos ?? 0 ?> usuarios activos en la última hora</span>
                                </div>
                                <span class="notification-time">Hoy</span>
                            </div>
                        </div>
                        <div class="notifications-footer">
                            <span class="notifications-count" id="notificationsCount">3 notificaciones</span>
                            <button class="btn-notifications-settings" onclick="openNotificationsSettings()">
                                <i class="fa-solid fa-sliders-h"></i> Configurar
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <!-- FILA INFERIOR: MÁS WIDGETS -->
            <div class="dashboard-bottom-row">
                <!-- WIDGET 5: PRÓXIMAS TAREAS -->
                <div class="widget-main widget-tasks">
                    <div class="widget-header">
                        <h3><i class="fa-solid fa-tasks"></i> Próximas Tareas</h3>
                        <button class="add-task" onclick="addNewTask()">
                    ...
                    <div class="widget-content">
                        <div class="tasks-list" id="tasksList">
                            <div class="task-item priority-high">
                                ...
                                <div class="task-details">
                                    <span class="task-title">Revisar solicitudes pendientes</span>
                                    <span class="task-desc"><?= $solicitudesPendientes ?? 0 ?> propiedades por aprobar</span>
                                </div>
                                <div class="task-meta">
                            ...
                            <div class="task-item priority-medium">
                                ...
                                <div class="task-details">
                                    <span class="task-title">Actualizar información del sistema</span>
                                    <span class="task-desc">Revisar estadísticas y métricas</span>
                                </div>
                                <div class="task-meta">
                            ...
                            <div class="task-item priority-low">
                                ...
                                <div class="task-details">
                                    <span class="task-title">Revisar logs del sistema</span>
                                    <span class="task-desc">Verificar actividad y errores</span>
                                </div>
                                <div class="task-meta">
                                    <span class="task-priority">Baja</span>
                                    <span class="task-date">Este mes</span>
                                </div>
                            </div>
                        </div>
                        <div class="tasks-progress">
                            ...
                            <span class="progress-text">1 de 3 tareas completadas</span>
                        </div>
                    </div>
                </div>

                <!-- WIDGET 6: ENLACES RÁPIDOS -->
                <div class="widget-main widget-links">
                    <div class="widget-header">
                        <h3><i class="fa-solid fa-link"></i> Enlaces Rápidos</h3>
                    </div>
                    <div class="widget-content">
                        <div class="links-grid">
                            <a href="#" class="link-card" onclick="mostrarSeccion('agregarpropiedad')">
                                <div class="link-icon">
                                    <i class="fa-solid fa-plus-circle"></i>
                                </div>
                                <div class="link-info">
                                    <span class="link-title">Agregar Propiedad</span>
                                    <span class="link-desc">Publicar nueva propiedad</span>
                                </div>
                                <i class="fa-solid fa-chevron-right link-arrow"></i>
                            </a>
                            
                            <a href="#" class="link-card" onclick="mostrarSeccion('usuarios')">
                                <div class="link-icon">
                                    <i class="fa-solid fa-user-cog"></i>
                                </div>
                                <div class="link-info">
                                    <span class="link-title">Gestionar Usuarios</span>
                                    <span class="link-desc">Administrar todos los usuarios</span>
                                </div>
                                <i class="fa-solid fa-chevron-right link-arrow"></i>
                            </a>
                            
                            <a href="#" class="link-card" onclick="mostrarSeccion('propiedadespublicadas')">
                                <div class="link-icon">
                                    <i class="fa-solid fa-building"></i>
                                </div>
                                <div class="link-info">
                                    <span class="link-title">Propiedades Publicadas</span>
                                    <span class="link-desc">Ver y gestionar propiedades</span>
                                </div>
                                <i class="fa-solid fa-chevron-right link-arrow"></i>
                            </a>
                            
                            <a href="#" class="link-card" onclick="mostrarSeccion('estadisticas')">
                                <div class="link-icon">
                                    <i class="fa-solid fa-chart-bar"></i>
                                </div>
                                <div class="link-info">
                                    <span class="link-title">Ver Estadísticas</span>
                                    <span class="link-desc">Análisis y reportes</span>
                                </div>
                                <i class="fa-solid fa-chevron-right link-arrow"></i>
                            </a>
                            
                            <a href="#" class="link-card" onclick="window.open('../index.php', '_blank')">
                                <div class="link-icon">
                                    <i class="fa-solid fa-external-link-alt"></i>
                                </div>
                                <div class="link-info">
                                    <span class="link-title">Ver Sitio Web</span>
                                    <span class="link-desc">Abrir sitio principal</span>
                                </div>
                                <i class="fa-solid fa-chevron-right link-arrow"></i>
                            </a>
                            
                            <a href="#" class="link-card" id="systemSettingsLink">
                                <div class="link-icon">
                                    <i class="fa-solid fa-cogs"></i>
                                </div>
                                <div class="link-info">
                                    <span class="link-title">Configuración</span>
                                    <span class="link-desc">Ajustes del sistema</span>
                                </div>
                                <i class="fa-solid fa-chevron-right link-arrow"></i>
                            </a>
                        </div>
                ...
                <!-- WIDGET 7: CALENDARIO MINI -->
                <div class="widget-main widget-calendar">
                    <div class="widget-header">
                        <h3><i class="fa-solid fa-calendar-alt"></i> Calendario</h3>
                        <div class="calendar-nav">
                            <button class="calendar-prev" id="calendarPrev">
                                <i class="fa-solid fa-chevron-left"></i>
                            </button>
                            <span class="calendar-month" id="currentMonth"><?= date('F Y') ?></span>
                            <button class="calendar-next" id="calendarNext">
                    ...
                    <div class="widget-content">
                        ...
                        <div class="calendar-events">
                            <div class="event-item today">
                                ...
                                <span class="event-title">Revisión diaria</span>
                                <span class="event-time"><?= date('H:i') ?></span>
                            </div>
                            <?php if ($solicitudesPendientes > 0): ?>
                            <div class="event-item urgent">
                                <div class="event-dot"></div>
                                <span class="event-title">Solicitudes pendientes</span>
                                <span class="event-count"><?= $solicitudesPendientes ?></span>
                            </div>
                            <?php endif; ?>
                            <div class="event-item upcoming">
                                <div class="event-dot"></div>
                                <span class="event-title">Backup semanal</span>
                                <span class="event-date">Viernes</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="dashboard-footer-main">
                <div class="footer-left">
                    <div class="system-info">
                        ...
                        <span>
                            <strong>RENTNONO Admin</strong> · 
                            Versión 2.0 · 
                            <span id="serverTime"><?= date('H:i:s') ?></span>
                        </span>
                    </div>
                </div>
                <div class="footer-right">
                    <div class="footer-stats">
                        <span class="footer-stat">
                            <i class="fa-solid fa-database"></i>
                            <span id="totalQueries">0</span> consultas
                        </span>
                        <span class="footer-stat">
                            <i class="fa-solid fa-memory"></i>
                            <span id="memoryUsage">0</span> MB
                        </span>
                        <span class="footer-stat">
                            <i class="fa-solid fa-user-friends"></i>
                            <span id="onlineUsers"><?= $usuariosActivos ?? 1 ?></span> en línea
                        </span>
                    </div>
                    <button class="btn-system-status" id="systemStatusBtn">
                        <i class="fa-solid fa-circle status-indicator-good"></i>
        ...
        <!-- SOLICITUDES PENDIENTES - VERSIÓN MEJORADA -->
            ...
            <div class="usuarios-header solicitudes-mejoradas">
                ...
                <div class="stats-rapidas-solicitudes">
                    ...
                    <div class="stat-card mini">
                        ...
                        <div>
                            <span class="stat-numero" id="solicitudesHoy"><?= $solicitudesHoy ?? 0 ?></span>
        ...
        <!-- ESTADÍSTICAS MEJORADAS - DASHBOARD DE MÉTRICAS -->
        <section id="estadisticas" class="seccion <?= $seccionActiva === 'estadisticas' ? 'visible' : '' ?>">
            <div class="dashboard-estadisticas-header">
                ...
                <div class="dashboard-controls">
                    ...
                    <!-- RANGO DE FECHAS (visible solo para personalizado) -->
            ...
            <div class="graficos-container">
                ...
                <div class="grafico-card grafico-grande">
                    ...
                    <div class="grafico-footer">
                        <span class="grafico-info"><i class="fa-solid fa-info-circle"></i> Últimos 7 días</span>
                    </div>
                </div>
                ...
                <!-- GRÁFICO 3: ESTADO DE SOLICITUDES -->
                <div class="grafico-card">
                    <div class="grafico-header">
            ...
            <div class="dashboard-footer">
                <div class="footer-info">
                    <i class="fa-solid fa-database"></i>
                    <span>Última actualización: <strong id="ultimaActualizacion"><?= date('d/m/Y H:i:s') ?></strong></span>
                </div>
                <div class="footer-stats">
                    <span class="footer-stat">
                        <i class="fa-solid fa-server"></i>
                        <span id="totalRegistros">0</span> registros totales
                    </span>
                    <span class="footer-stat">
                        <i class="fa-solid fa-bolt"></i>
                        <span id="tiempoRespuesta">0.8</span>s tiempo de respuesta
                    </span>
                </div>
            </div>
        </section>
